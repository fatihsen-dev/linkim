   ...
   return (
      <div className="flex h-full gap-10 pb-10">
         <div className="h-full p-10 max-w-[300px] w-full flex flex-col rounded-md border border-white/10 items-center gap-5 bg-white/5 backdrop-blur-md">
            <Avvvatars size={70} style="shape" value={user?.email || ""} />
            <div className="text-center">
               <h3 className="text-xl leading-4 font-semibold text-white">{user?.user_metadata.displayName}</h3>
               <span className="leading-4 text-white/50">{user?.user_metadata.username}</span>
            </div>
         </div>
